    ...
    mycartsInt() {
      ...
      GET_MYCARTS(params, function(res) {
        if (res.statusCode === 1) {
          ...
          console.log("shoppingCart mycartsInt:" + res.data);
        }
      });
    },
    defState() {
      ...
      _this.getShCartData.map((v, i, arr) => {
        ...
        allPrice +=
          Number(_this.getShCartData[i].marketprice) *
          Number(_this.getShCartData[i].total);
    ...
    nowChecked(v, i) {
      ...
      _this.getShCartData.map((v, i, arr) => {
        if (_this.getShCartData[i].isChecked === true) {
          ...
          myArr.length == _this.getShCartData.length
            ? (_this.isTrue = true)
            : (_this.isTrue = false);
        } else {
          _this.isTrue = false;
